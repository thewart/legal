functions {
  real scalemixcdf(real x, vector w, vector loc, vector scale) {
    ...
    for (i in 1:K)
      y += w[i]*logistic_cdf(x, loc[i], scale[i]);
    return y;
  }
...
transformed parameters {
  ...
  loc = rep_vector(0,K);
  scale = sprop * sqrt(1 - 3*pi()^-2 * dot_product(w, loc .* loc)) / sqrt(dot_product(w, sprop .* sprop));
...
model {
    ...
    sprop ~ dirichlet(rep_vector(1,K));
    // loc_dist ~ normal(0,0.5);
