...
parameters {
  ...
  vector[P2] lambda_subj_raw[Nsub];
...
transformed parameters {
  ...
  for (i in 1:Nsub) lambda[i] = sigma_lambda_mu * lambda_mu_raw + sigma_lambda_subj * lambda_subj_raw[i];
      
  // get linear predictor
  for (i in 1:N) {
    theta[i] = dot_product(X[i], beta[S[i],C[i]]) + dot_product(Z[i],lambda[S[i]]);
    if (cens[i] == 0)
...
model {
  ...
  eta ~ normal(0., 2.5);
  tau ~ normal(0., 2.5);
  sigma_lambda_subj ~ normal(0., 2.5);
